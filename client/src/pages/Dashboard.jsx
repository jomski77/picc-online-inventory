  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const lowStockRes = await fetch('/api/items/low-stock/all', {
          credentials: 'include',
        });
        ...
        const stockRes = await fetch('/api/stock?limit=5&populate=createdBy', {
          credentials: 'include',
        });
        const stockData = await stockRes.json();
        
        const usageRes = await fetch('/api/stockUsage?limit=5&populate=createdBy', {
          credentials: 'include',
        });
        const usageData = await usageRes.json();
        
        // Fetch all usage data to calculate total used
        const allUsageRes = await fetch('/api/stockUsage?populate=createdBy', {
          credentials: 'include',
        });
